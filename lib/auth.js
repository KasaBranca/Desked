...
class AuthManager {
  constructor() {
    ...
    this.passwordHash = this._hashPassword(config.password);
  }
  ...
  authenticate(password, ip) {
    ...
    if (!this._safeCompare(hash, this.passwordHash)) {
  ...
  /** Verify a password without touching lockout state or creating a session. */
  checkPassword(password) {
    return this._safeCompare(this._hashPassword(password), this.passwordHash);
  }

  /** Replace the active password hash (caller is responsible for persistence). */
  setPassword(password) {
    this.passwordHash = this._hashPassword(password);
  }

  /** Invalidate every session except the given token. */
  revokeOtherSessions(keepToken) {
